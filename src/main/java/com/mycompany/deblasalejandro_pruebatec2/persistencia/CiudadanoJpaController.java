...
public class CiudadanoJpaController implements Serializable {
    ...
    // Utilizar JPQL para realizar la consulta por nombre
    public Ciudadano findCiudadanoNombre(String nombre) {
        EntityManager em = getEntityManager();
        try {          
            String jpql = "SELECT c FROM Ciudadano c WHERE c.nombre = :nombre";
